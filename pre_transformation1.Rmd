...
* Always return a data frame
    ...
    * (Never modify in place) - EH: what does this mean???
...
One can extract rows that meet logical criteria by using the `filter` command. The first argument is the name of the data frame with subsequent argument(s) being logical expressions. E.g. one subset the minke data containing only data from 2005 and later by:
...
All but the year 2005 would be:
```{r, eval = FALSE}
# not run
filter(d, year != 2005)
```
...
```{r, eval = FALSE}
# not run
filter(d, year %in% c(2005, 2007))
```

Which would be the same as:
```{r, eval = FALSE}
# not run
filter(d, year == 2005 | year == 2007)
```
