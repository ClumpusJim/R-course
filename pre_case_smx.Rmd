...
This case example demonstrates how to calculate standardized survey indices from SMB using the vatican approach.
...
SMBSTATIONSSTRATA <- 
  STODVAR.all %>%
  select(synis_id = synis.id, strata = oldstrata, lon, lat)
dbWriteTable(con, name = "SMBSTATIONSSTRATA", value = SMBSTATIONSSTRATA, overwrite = TRUE)
detach("file:/net/hafkaldi/export/u2/reikn/R/SurveyWork/SMB/Stations.rdata")
OLDSTRATAAREA <- tbl(con, "oldstataarea") %>% collect(n = Inf)
dbWriteTable(con, name = "OLDSTRATAAREA", value = OLDSTRATAAREA, overwrite = TRUE)
```
